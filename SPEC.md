...
B8|2026-06-24|Deep testing session: added 327 new tests across 22 crates (was 168, now 495). Tests include: 170+ unit/integration edge-case tests for trivial boundaries (unicode, concurrency, error paths, TTL, symlinks, metachar injection); 40 e2e live Docker tests in new `glia-e2e` crate against Glia's own HelixDB/OpenBao/Redis/Hub stack; 30 real-Docker tests replacing `StubOpenBao`/`MockUnwrapper`/`StubCatalog` usage in glia-bao/glia-cache/glia-sandbox/glia-helix. Bug fixes implemented: (1) Exit code 2 for Hub-unreachable (SPEC V15) — `process::exit(2)` mapping in `crates/glia-cli/src/main.rs` with `is_hub_unreachable()` checking `SyncError::HubUnreachable`, `HelixError::Connect`, `HelixError::Http`. (2) Exit code 3 for AUTH_TIMEOUT (SPEC V14) — `is_auth_timeout()` + `process::exit(3)`. (3) `glia context` subcommand added — previously referenced by Claude hooks but missing from `Cmd` enum. Wired `glia-context` + `glia-synth` as CLI deps. (4) Hooks settings.json merge — `merge_hooks_into_settings()` preserves existing hooks and settings, deduplicates Glia hooks. Changed `ClaudeHookEntry` to match Claude Code schema (`{matcher, hooks: [{type, command}]}`). (5) Hub graceful shutdown on SIGINT — `tokio::select!` with `ctrl_c()`. (6) `chunk ingest --all/--changed` dead flags removed — simplified `ChunkOp::Ingest` variant. (7) `save-skill` output includes `id`, `source`, `stacks`, `name` (was just `id`). (8) Symlink escape detection — `Fs::resolve` canonicalizes both root and path, rejects if canonical path escapes canonical root. (9) `bridge` `run_bridge` refactored — `run_bridge_with_io(cfg, stdin, stdout)` accepts generic `R: AsyncRead, W: AsyncWrite`. (10) `action` auth timeout now returns `Err("AUTH_TIMEOUT after ...")` and exits code 3 (was always `Ok(())` + exit 0). (11) Docker compose healthcheck for helixdb — `\\dev/tcp/6969` check + `glia-hub` uses `service_started` instead of `service_healthy` for helixdb (the minimal enterprise-dev image has no wget/curl for healthcheck). Isolated Glia stack: 4 containers, all Glia-owned (`glia-helixdb-1`, `glia-openbao-1`, `glia-redis-1`, `glia-glia-hub-1`). New `crates/glia-e2e/` crate with `tests/{common,live_helix,live_openbao,live_redis,live_hub,live_auth,cross_cutting,bug_docs}.rs`. All 495 tests pass, clippy clean. Remaining known issues: `cache` TTL=0 divergence (InMemory: sub-second OK; Redis: clamped to 1s — by design), `serve(addr)` no 0.0.0.0 option (by design for tests)