...
id|date|cause|fix
B1|2026-06-23|`glia init` installs pre-push hook calling `glia chunk ingest --all` but `glia chunk` is not a registered subcommand in `crates/glia-cli/src/main.rs` — hook fails on every commit|pick one: (a) implement `glia chunk ingest` subcommand calling `crates/glia-chunk` library, (b) stub `glia chunk` as no-op, (c) remove the hook call from template
B2|2026-06-23|Default catalog URL `AnomalyCo/community-catalog` is dead (404) — `GitHubCatalog` cannot resolve tools, no `community::*` skills register|pick: (a) point default at `Vellixia/community-catalog` (existing T21 repo), (b) document that user must `glia init --catalog <url>`
...
B4|2026-06-23|SurrealDB rust client v2.6.5 cannot resolve `ws://127.0.0.1:8000` or `ws://localhost:8000` on Windows (`IO error: No such host is known. (os error 11001)`) even though server accepts curl WS upgrade on same address|blame Windows resolver or TLS-SNI mismatch in `surrealdb-core` 2.6.5. Workaround: run CLI on WSL/Linux for sync e2e, or wait for upstream patch. Blocks F13+F22 e2e (sync LWW + offline queue)
